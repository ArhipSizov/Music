...
const ImageUpload = (props: IMGProps) => {
    ...
    const cropper = storageRef.current?.cropper;
    ...
    return ( 
        <div>
            ...
            <div id="img-display">
                ...
                <img className="img_cropt" id={props.round ? "round" : ""} src={croppedImage} />
                <div id="options-row" style={{width: props.width  || 375 + "px"}}>
                    {/* <button id="edit-button" onClick={showEditor}>Edit</button> */}
                    <button id="save-button" style={{backgroundColor: props.color ? props.color : "dodgerblue"}} onClick={() => saveImage()}>Сохранить</button>
                </div>
            </div>
            }
